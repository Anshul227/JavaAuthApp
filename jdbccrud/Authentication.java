...
public class Authentication {
	...
	public static boolean passCheck(String userId,String pass,Connection conn) {
		...
		try {
			PreparedStatement stm = conn.prepareStatement("select password"+table+"where user_id = "+userId);
...
//		conn = null;
		userId = "'"+userId+"'";
		try {
			PreparedStatement stm = conn.prepareStatement("select id "+table+"where user_id = "+userId );
			ResultSet rs = stm.executeQuery();
			if (!rs.next()) {
				return false;
			}
